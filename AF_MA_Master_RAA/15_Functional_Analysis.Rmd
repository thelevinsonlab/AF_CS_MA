...
```{r}
# Load the CS data
RAA_gene_rank <- read_excel("~/PhD Heidelberg/Projects/AF_MA_Master_RAA/SupplementalTable2.xlsx")

# Making some transformations to generate the ranked gene list 
RAA_gene_rank$fcSign=sign(RAA_gene_rank$mean_lfc)
RAA_gene_rank$logP=-log10(RAA_gene_rank$fisher_pvalue)
RAA_gene_rank$score=RAA_gene_rank$logP/RAA_gene_rank$fcSign
RAA_gene_rank <- RAA_gene_rank[order(-RAA_gene_rank$score),]

# Generating the ranked gene list
RAA_gene_list <- RAA_gene_rank$score
names(RAA_gene_list) <- RAA_gene_rank$gene
```
...
gsea_results_bp <- GSEA(
  geneList = RAA_gene_list, # Ordered ranked gene list
...
CS_rank <- RAA_gene_rank %>% column_to_rownames("gene") %>%
...
CS_rank_pathway <- RAA_gene_rank %>% column_to_rownames("gene") %>%
  dplyr::select(mean_t) %>% 
  filter(!is.na(mean_t)) %>% as.matrix()
```
...
# Save the workbook
saveWorkbook(wb, "Functional_analysis_results_RAA.xlsx", overwrite = TRUE)
```


```{r}
# Filter and format the results of each process accordingly and create plots
...
pdf("./Functional_analysis_RAA_results.pdf",
    width = 18.3,
    height = 10.21)
